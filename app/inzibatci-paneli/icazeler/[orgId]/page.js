...
const STATUS_LABELS = {
    can_view: 'Baxış', can_create: 'Yaratma', can_edit: 'Redaktə', can_approve: 'Təsdiq',
};

function PermissionStatus({permission}) {
    const activeStatuses = Object.entries(STATUS_LABELS)
    ...
    return (<Box
        ...
        >
            {activeStatuses.map((label) => (<Chip
                    key={label}
                    size="small"
                    label={label}
                    sx={{
                        height: 22, fontSize: 10.5, backgroundColor: GOV.pageBg, color: GOV.textMuted,
                    }}
                />))}
        </Box>);
}
...
export default function Page() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                const [orgRes, employeesRes] = await Promise.all([service_api.get(NEXT_API_ENDPOINTS.ORGANIZATIONS.DETAIL(params.orgId)),
                ...
                const permResults = await Promise.all(emps.map((e) => service_api
                    .get(`${NEXT_API_ENDPOINTS.PERMISSIONS.USER_PERMISSIONS}?user=${e.id}`)
    ...
    return (<AppShell>
            ...
            >
                ...
                >
                    ...
                    <Link
                        component="button"
                        onClick={() => router.push(APP_ROUTES.INZIBATCI_ICAZELER)}
                        sx={{
                            fontSize: 12.5, color: GOV.textMuted, textDecoration: 'none',
                        }}
                    >
                        ...
                        {employees.map((emp, i) => {
                            ...
                            return (<Box
                                ...
                                >
                                    <Box
                                        sx={{
                                            display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 220,
                                        }}
                                    >
                                        <Avatar
                                            sx={{
                                    ...
                                    <Box
                                        sx={{
                                            display: 'grid', gap: 0.75, flexGrow: 1, minWidth: 300,
                                        }}
                                    >
                                        {perms.length === 0 ? (<Typography
                                                sx={{
                                                    fontSize: 12.5, color: GOV.textMuted,
                                                }}
                                            >
                                                Heç bir modula icazəsi yoxdur.
                                            </Typography>) : (perms.map((p) => (<Box
                                                    key={p.id}
                                                    sx={{
                                                        display: 'flex',
                                                        alignItems: 'center',
                                                ...
                                                >
                                                    <Typography
                                                        sx={{
                                                            fontSize: 12, fontWeight: 600, color: GOV.textPrimary,
                                                        }}
                                                    >
                                                        {p.module_title}
                                                    </Typography>
