...
const TYPE_META = {
    gomrukden_azadolma: {title: 'Gömrükdən Azadolma icazə sənədi yarat'},
    edvden_azadolma: {title: 'ƏDV-dən Azadolma icazə sənədi yarat'},
};
...
export default function Page() {
    ...
    const docType = params?.type === 'edvden_azadolma' ? 'edvden_azadolma' : 'gomrukden_azadolma';
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            router.push(APP_ROUTES.EDV_GUZESTI);
        } catch (e) {
            enqueueSnackbar(handleError(e), {variant: 'error'});
        } finally {
            setSubmitting(false);
        ...
        <AppShell>
            <Box sx={{maxWidth: "90%", mx: 'auto', px: {xs: 2, md: 4}, py: {xs: 4, md: 6}}}>
                ...
                <Box sx={{display: 'flex', justifyContent: 'flex-end', gap: 1.5}}>
                    <Button
                        onClick={() => router.push(APP_ROUTES.EDV_GUZESTI)} disabled={submitting}
