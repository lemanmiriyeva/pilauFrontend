    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                ...
                // "Subyekt adı" avtomatik sahəsi müəssisənin adı ilə doldurulur.
                if (org?.full_name) {
                    setFormValues((prev) => ({...prev, subyekt_adi: org.full_name}));
                }
            } catch (e) {
        ...
        <AppShell>
            <Box sx={{maxWidth: "90%", mx: 'auto', px: {xs: 2, md: 4}, py: {xs: 4, md: 6}}}>
                ...
                <Box sx={{display: 'flex', justifyContent: 'flex-end', gap: 1.5}}>
                    <Button
                        onClick={() => router.push(APP_ROUTES.ISTEHSAL)} disabled={submitting}
