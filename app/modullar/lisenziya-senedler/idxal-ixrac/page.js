        ...
        const timeout = setTimeout(() => {
            (async () => {
                ...
                try {
                    ...
                    params.set('doc_type', 'ixrac,idxal');
        ...
        <AppShell>
            <Box sx={{maxWidth: "90%", mx: 'auto', px: {xs: 2, md: 4}, py: {xs: 4, md: 6}}}>
                <Typography sx={{fontSize: 12.5, color: GOV.textMuted, mb: 1}}>
                    ...
                    {' / '}
                    <span style={{fontWeight: 700, color: GOV.textPrimary}}>İdxal/İxrac icazə sənədi</span>
                </Typography>
                ...
                <Box sx={{display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2, mb: 3}}>
                    <Box>
                        <Typography sx={{fontSize: 24, fontWeight: 800, color: GOV.textPrimary}}>
                            İdxal / İxrac icazə sənədi
                        </Typography>
                        <Typography sx={{fontSize: 13, color: GOV.textMuted, mt: 0.5}}>
                            Müəssisənizə aid bütün idxal / ixrac icazə sənədlərini idarə və izləyin.
                        </Typography>
                    </Box>
                    <Button
                        variant="contained" startIcon={<AddIcon/>}
                        onClick={() => router.push(APP_ROUTES.IDXAL_IXRAC_YENI)}
                        sx={{
                ...
                <Box sx={{display: 'flex', gap: 1.5, mb: 2.5, flexWrap: 'wrap'}}>
                    <TextField
                        size="small" placeholder="Lisenziya axtar..." value={search}
                            ...
                            <Box component="table" sx={{width: '100%', borderCollapse: 'collapse'}}>
                                <Box component="thead">
                                    <Box component="tr" sx={{borderBottom: `1px solid ${GOV.cardBorder}`}}>
                                        {['Sənəd nömrə, başlıq', 'Kateqoriya', 'Verilmə tarixi', 'Bitmə tarixi', 'Status', ''].map((h) => (
                                        ...
                                        <Box component="tr" key={row.id} sx={{
                                            ...
                                            <Box component="td" sx={{px: 2.5, py: 1.75, textAlign: 'right'}}>
                                                <Button
                                                    ...
                                                    onClick={() => router.push(`${APP_ROUTES.IDXAL_IXRAC}/${row.id}`)}
                                                    sx={{
